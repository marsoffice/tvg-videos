...
{
    public class SpeechResultConsumer
    {
        private readonly IConfiguration _config;
        private readonly IMapper _mapper;

        public SpeechResultConsumer(IConfiguration config, IMapper mapper)
        {
            _config = config;
            _mapper = mapper;
        }

        [FunctionName("SpeechResultConsumer")]
        public async Task Run(
            [QueueTrigger("speech-result", Connection = "localsaconnectionstring")] CloudQueueMessage message,
            ...
            ILogger log)
        {
            var response = Newtonsoft.Json.JsonConvert.DeserializeObject<SpeechResult>(message.AsString,
            ...
            try
            {
                ...
                else
                {
                    ...
                    var mergeEntity = new VideoEntity
                    {
                        ...
                        ETag = "*",
                        SpeechFile = response.FileLink,
                        FinalFileDurationInMillis = response.TotalDurationInMillis,
                        Durations = string.Join(",", response.IndividualDurationsInMillis),
                        SpeechDone = true
                    };
