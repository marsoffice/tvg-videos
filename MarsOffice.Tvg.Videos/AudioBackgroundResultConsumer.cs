...
{
    public class AudioBackgroundResultConsumer
    {
        private readonly IConfiguration _config;
        private readonly IMapper _mapper;

        public AudioBackgroundResultConsumer(IConfiguration config, IMapper mapper)
        {
            _config = config;
            _mapper = mapper;
        }

        [FunctionName("AudioBackgroundResultConsumer")]
        public async Task Run(
            [QueueTrigger("audiobackground-result", Connection = "localsaconnectionstring")] QueueMessage message,
            ...
            ILogger log)
        {
            var response = Newtonsoft.Json.JsonConvert.DeserializeObject<AudioBackgroundResult>(message.Text,
            ...
            try
            {
                ...
                else
                {
                    ...
                    var mergeEntity = new VideoEntity
                    {
                        ...
                        ETag = "*",
                        AudioBackgroundDone = true,
                        AudioBackgroundFile = response.FileLink
                    };
